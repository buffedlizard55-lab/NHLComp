...
SOURCES: tuple[SourceSpec, ...] = (
    ...
    SourceSpec(
        source_id="kalshi.period",
        name="Kalshi NHL period markets (KXNHL1P, KXNHL2P, KXNHL3P)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL1P&status=open",
        data_type="binary first/second/third period winner contracts: yes/no offers, floor_strike NULL, suffix OT for OT variant, team naming same as moneyline",
        nhl_relevance="Period-level moneyline: a different settlement (period score, not final score) and a potentially softer market than full-game",
        historical_depth="Verified negative on 2026-09-21: /series lists KXNHL1P but /markets?status=open returned 0 contracts and /historical/markets?series_ticker=KXNHL1P returned 0 contracts. Polled every run via kalshi_series_watch.",
        live_available=False,
        update_frequency="When listed, continuous while open",
        api_available=True,
        auth_required="none for market data reads",
        cost="free",
        genuinely_free="yes",
        usage_limits="Paginated with cursor; empty on 2026-09-21, so nothing to page yet",
        licensing="Kalshi API terms apply",
        provenance="Kalshi (first party exchange)",
        reliability="high",
        accuracy="exchange-grade when listed",
        granularity="per period per game, one contract per team",
        automated_access="yes",
        known_limits="No open or settled contracts observed on 2026-09-21, so no price history exists to backtest against. Settlement would be from game_period_scores (period goals from /v1/score), which this project already ingests for research.",
        notes="Registered as a market family this project is ready to trade but that is currently empty. WATCH_SERIES in kalshi.py includes KXNHL1P/2P/3P and records each poll as a verified negative in kalshi_series.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL1P&limit=2&status=open",),
    ),
    SourceSpec(
        source_id="kalshi.period_total",
        name="Kalshi NHL period totals (KXNHL1PTOTAL, KXNHL2PTOTAL, KXNHL3PTOTAL)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL1PTOTAL&status=open",
        data_type="binary 'Over k.5 goals in period N' contracts: floor_strike + strike_type, YES=Over, NO=Under, one per strike per game",
        nhl_relevance="Period-level totals: scores are low (0-2 typical) so lines are 0.5/1.5/2.5 and pricing differs materially from full-game totals",
        historical_depth="No contracts observed on 2026-09-21 (preseason). The series are listed in /series?category=Sports&tags=Hockey per the live probe, but empty.",
        live_available=False,
        update_frequency="When listed, continuous",
        api_available=True,
        auth_required="none for reads",
        cost="free",
        genuinely_free="yes",
        usage_limits="Paginated; empty currently",
        licensing="Kalshi API terms apply",
        provenance="Kalshi (first party exchange)",
        reliability="high",
        accuracy="exchange-grade when listed",
        granularity="per period per strike per game",
        automated_access="yes",
        known_limits="No price history yet, so forward-test only when listed. Settlement would be from game_period_scores (period goals). No period-level expected-goals model exists here yet, so nothing is bet even when a quote appears, until a model exists.",
        notes="WATCH_SERIES includes these three. Blocked until a period model exists.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL1PTOTAL&limit=2&status=open",),
    ),
    SourceSpec(
        source_id="kalshi.team_total",
        name="Kalshi NHL team totals (KXNHLTEAMTOTAL)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLTEAMTOTAL&status=open",
        data_type="binary team-specific over/under contracts, if listed: floor_strike + strike_type per team",
        nhl_relevance="Team totals isolate one team's scoring from the opponent's, different from full-game totals",
        historical_depth="Not observed in /series listing on 2026-09-21; candidate series probed via WATCH_SERIES pattern",
        live_available=False,
        update_frequency="unknown",
        api_available=True,
        auth_required="none",
        ...
        provenance="Kalshi (first party exchange)",
        reliability="unknown",
        accuracy="unknown until observed",
        granularity="per team per strike per game",
        automated_access="yes",
        known_limits="No verified contract shape yet. If the series appears, settlement is home_score / away_score individually, already stored.",
        notes="Added for completeness per brief requirement to research team totals. Not traded until shape verified.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/series?category=Sports&tags=Hockey",),
    ),
    SourceSpec(
        source_id="kalshi.regulation",
        name="Kalshi NHL regulation markets (KXNHLREG, KXNHL60MIN)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLREG&status=open",
        data_type="binary regulation-time winner contracts (win in regulation, not OT/SO), if listed",
        nhl_relevance="Regulation vs full-game moneyline differ by the OT mass; a regulation win pays only when decided in REG",
        historical_depth="Not observed in 2026-09-21 series listing; candidate for future listing",
        live_available=False,
        update_frequency="unknown",
        api_available=True,
        auth_required="none",
        ...
        granularity="per game",
        automated_access="yes",
        known_limits="No contracts observed. Settlement would be lastPeriodType==REG and winner. No price history yet.",
        notes="Required by brief: regulation market. Blocked until contracts appear and settlement rule verified.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/series?category=Sports&tags=Hockey",),
    ),
    SourceSpec(
        source_id="kalshi.player_props",
        name="Kalshi NHL player props (KXNHLGOAL, KXNHLANYGOAL, KXNHLAST, KXNHLPTS, KXNHLFIRSTGOAL)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLGOAL&status=open",
        data_type="binary player to score a goal / record an assist / record points contracts",
        nhl_relevance="Player props are typically softer than game markets; require player-level projection model",
        historical_depth="Series listed in /series?category=Sports&tags=Hockey on 2026-09-21 (e.g. KXNHLGOAL) but with 0 open contracts. No historical-tier contracts observed.",
        live_available=False,
        update_frequency="When listed, continuous",
        api_available=True,
        auth_required="none for reads",
        ...
        automated_access="yes",
        known_limits="No verified player-level data feed exists for pre-game deployment, and no player-level model exists here yet. Even when listed, blocked until both exist. Also requires mapping player names to NHL player_ids.",
        notes="WATCH_SERIES includes KXNHLGOAL, KXNHLANYGOAL, KXNHLAST, KXNHLPTS, KXNHLFIRSTGOAL. Verified negative on 2026-09-21: 0 contracts. No price history.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLGOAL&limit=2&status=open",),
    ),
    SourceSpec(
        source_id="kalshi.goalie_props",
        name="Kalshi NHL goalie props (KXNHLSAVES)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLSAVES&status=open",
        data_type="binary goalie saves over/under contracts, if listed: floor_strike = saves line",
        nhl_relevance="Goalie saves prop isolates goalie workload and shot volume",
        historical_depth="Series KXNHLSAVES listed in /series on 2026-09-21 but 0 open contracts; no historical-tier contracts observed. Captured as verified negative.",
        live_available=False,
        update_frequency="When listed, continuous",
        api_available=True,
        auth_required="none for reads",
        ...
        reliability="high",
        accuracy="exchange-grade when listed",
        granularity="per goalie per game",
        automated_access="yes",
        known_limits="No verified pre-game starter source, so even if listed, forward-test only when starter known. No saves projection model exists here yet.",
        notes="WATCH_SERIES includes KXNHLSAVES. Verified negative 2026-09-21: 0 contracts. Blocked until starter feed + model.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLSAVES&limit=2&status=open",),
    ),
    SourceSpec(
        source_id="kalshi.futures",
        name="Kalshi NHL futures (KXNHL)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL&status=open",
        data_type="binary Stanley Cup champion / conference champion / division winner contracts",
        nhl_relevance="Futures are long-horizon, require season-level model and bankroll management for long open exposure",
        historical_depth="KXNHL futures have been listed (observed in series listing), but not walked for history in this project; futures settlement is far in future",
        live_available=True,
        update_frequency="Continuous while listed",
        api_available=True,
        auth_required="none for reads",
        cost="free",
        genuinely_free="yes",
        usage_limits="Paginated",
        licensing="Kalshi API terms apply",
        provenance="Kalshi (first party exchange)",
        reliability="high",
        accuracy="exchange-grade",
        granularity="per team per market, season-level",
        automated_access="yes",
        known_limits="Not traded here: requires season simulation model, long-horizon bankroll lockup, and settlement months in future. Listed as candidate for future research.",
        notes="Brief requires futures research. Registered but blocked: no season model exists.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHL&limit=2&status=open",),
    ),
    SourceSpec(
        source_id="kalshi.alternate",
        name="Kalshi alternate lines (KXNHLTOTAL, KXNHLSPREAD multi-rung)",
        url="https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLTOTAL&status=open",
        data_type="Multiple strikes per game: 1.5 through 8.5 on totals, 1.5/2.5/3.5 on puck line; each strike is an alternate line",
        nhl_relevance="Alternate lines are already the primary mechanism for totals and puck line: trading the 5.5 vs 6.5 vs 7.5 rung is explicitly an alternate-line strategy",
        historical_depth="Verified 2026-09-21: 8 strikes offered on 2026-09-24 slate (1.5-8.5) for totals, 2-3 rungs for puck line. Historical tier back to 2026 Stanley Cup Final.",
        live_available=True,
        update_frequency="Continuous",
        api_available=True,
        auth_required="none",
        cost="free",
        genuinely_free="yes",
        usage_limits="Candle history per rung; volume per rung",
        licensing="Kalshi API terms apply",
        provenance="Kalshi (first party exchange)",
        reliability="high",
        accuracy="exchange-grade",
        granularity="per strike per game",
        automated_access="yes",
        known_limits="Only YES side has candle history; NO side forward-only. Rung index digit is not the line, must read floor_strike.",
        notes="This source documents that alternate lines are already traded via the ladder mechanism in TotalsStrategy and PuckLineStrategy, with target_strike declared before seeing prices.",
        probe_urls=("https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNHLTOTAL&limit=5&status=open",),
    ),
